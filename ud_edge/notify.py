...
def format_opportunity_message(
    *,
    ...
    title = f"UD EDGE | {platform} | {player} | {delta_pp:+.1f}pp"
    body = (
        f"PLACE ON -> {platform}\n"
        f"{player} - {pick}\n"
        f"{match} - tips in {tips}\n"
        f"UD {ud_pct:.1%} -> {sharp_book} {sharp_pct:.1%} ({delta_pp:+.1f}pp)\n"
        f"Signal only from {sharp_book}. Open {platform} to enter the slip."
    )
    return title, body
...
def send_telegram(
    ...
    """Send via Telegram bot. Reads TELEGRAM_BOT_TOKEN + TELEGRAM_CHAT_ID from env if not passed."""
    ...
    text = f"*{title}*\n{body}"
    r = requests.post(url, json={"chat_id": cid, "text": text, "parse_mode": "Markdown"}, timeout=15)
    r.raise_for_status()
    return True
